...
class HashTable(object):
    ...
    def load_factor(self):
        ...
        return self.size/len(self.buckets) if self.size else 0
    ...
    def _resize(self, new_size=None):
        ...
        Best and worst case running time: O(n) under what conditions? because we must move over all items
        Best and worst case space usage: ??? what uses this memory? [TODO]"""
        # If unspecified, choose new size dynamically based on current size
        if new_size is None: # O(1)
            new_size = len(self.buckets) * 2 # O(1)
             # Double size
        # Option to reduce size if buckets are sparsely filled (low load factor)
        elif new_size is 0:
            new_size = len(self.buckets) / 2  # Half size
        # TODO: Get a list to temporarily hold all current key-value entries
        temporary_entries_list = self.items() # O(b) with b being the number of buckets => O(n) 
        # TODO: Create a new list of new_size total empty linked list buckets
        self.__init__(new_size) #O(2b) => O(b)
        # TODO: Insert each key-value entry into the new list of buckets,
        # which will rehash them into a new bucket index based on the new size
        
        for entry in temporary_entries_list: #O(n)
            key = entry[0]
            value = entry[1]
            # self.buckets[self._bucket_index(key)].append((key, value))
            self.set(key, value) #O(l) where l is the average length of the linked list
